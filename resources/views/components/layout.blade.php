...
@auth
    ...
    {{--    </aside>--}}
    <!-- Burger Icon -->
    <div id="burgerIcon" class="inline absolute text-2xl text-gray-900 h-full cursor-pointer p-4 lg:hidden">
        &#9776;
    </div>

    <!-- Sidebar -->
    <aside id="sidebar"
           class="absolute bg-blue-900 lg:block lg:w-64 w-48 h-full transition-transform transform  lg:translate-x-0 z-50">
        <nav class="flex flex-col space-y-4 pb-5 px-2 mt-6">
            <a href="/profile"
               class="flex flex-col justify-center items-center mb-3 text-white hover:text-blue-600">
                <img src="{{ asset('/images/profile-icon-design-free-vector.jpg') }}" alt=""
                     class="rounded-full w-20 h-20 ">
                <span
                    class="text-lg font-bold text-center sidebar-label text-white montserrat-regular">{{ Auth::user()->getFirstNameAndLastNameAbbreviation() }}</span>
            </a>
            <x-nav-link href="/" class="mb-2" :active="request()->is('/')">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                     stroke="black" class="size-6 sidebar-icon">
    ...
    <script>
        ...
        function toggleSidebar() {
            sidebar.classList.toggle('hidden'); // Show or hide the sidebar on mobile
            sidebar.classList.toggle('translate-x-full'); // Slide in/out the sidebar on mobile
        }
